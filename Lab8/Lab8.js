...
function wojewShow(sprw){
    if(sprw.value==="Polska"){ //źle zrobiona zmiana na select
    document.getElementById('wojew').replaceWith('<select id="wojew" name="wojew">'+'<option>Podkarpacie</option>'+'</select>')
...
form.addEventListener('submit', function (event) {
  ...
  if (!validateBirthdate(birthdateInput.value)) {
    document.getElementById("birthdate-error").innerHTML = "Wymagany wiek to ponad 18 lat  ";
    return;
  }else{
